...
const DashboardView = {
    data: {
        ...
        avgTicket: 0,
        topProducts: [],
        cashStatus: { efectivo: 0, tarjeta: 0, transferencia: 0, qr: 0 },
        businessOpen: false,
        alerts: {
            lowStock: [],
            overdueDebt: [],
    ...
    async render() {
        ...
        return `
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(210px, 1fr)); gap: 1rem; margin-bottom: 1.5rem;">
                ...
                <!-- Ganancia Neta Estimada del Día -->
                <div class="db-kpi" style="border-left-color: #10b981; animation-delay: 0.05s;" onclick="app.navigate('reports')">
                    <div class="db-kpi-bg-icon">📈</div>
                    <div class="db-kpi-label" style="color:#10b981;">Ganancia Estimada Hoy</div>
                    <div class="db-kpi-value" style="color:#0f172a;">${formatCLP(this.data.todayProfit, true)}</div>
                    <div class="db-kpi-sub" style="color:#64748b;">Ventas neto − costo neto vendido</div>
                </div>
                ...
                <!-- Deuda Clientes Fiados -->
                <div class="db-kpi" style="border-left-color: #8b5cf6; animation-delay: 0.1s; cursor: pointer;" onclick="app.navigate('customers')">
                    <div class="db-kpi-bg-icon">💳</div>
                    <div class="db-kpi-label" style="color:#8b5cf6;">Cuentas por Cobrar</div>
                    <div class="db-kpi-value" style="color:#0f172a;">${formatCLP(this.data.totalCustomerDebt, true)}</div>
                    <div class="db-kpi-sub" style="color:#64748b;">Fiados por cobrar a clientes</div>
                </div>
            ...
            <div class="db-section" style="animation-delay: 0.18s; margin-bottom: 1.25rem;">
                <div class="db-section-title" style="justify-content: space-between;">
                    <span>📊 Tendencia de Ventas (Últimos 7 Días)</span>
                    <span style="font-size:0.75rem; color:#64748b; font-weight:600;">Ventas acumuladas por día</span>
                </div>
    ...
    _renderWeeklyChart() {
        ...
        if (days.length === 0) return '<div style="color:#94a3b8; padding:1rem; text-align:center;">Sin datos suficientes</div>';
        const maxVal = Math.max(...days.map(d => d.total), 1);
        
        return `
            <div style="display: flex; align-items: flex-end; justify-content: space-between; gap: 0.75rem; height: 130px; padding-top: 1rem;">
                ${days.map(d => {
                    const heightPct = Math.max(8, Math.round((d.total / maxVal) * 100));
                    const isToday = d.dateStr === new Date().toISOString().slice(0, 10);
                    const barBg = isToday ? 'linear-gradient(180deg, #6366f1 0%, #4f46e5 100%)' : 'linear-gradient(180deg, #a5b4fc 0%, #818cf8 100%)';
                    return `
                        <div style="flex:1; display:flex; flex-direction:column; align-items:center; height:100%; justify-content:flex-end;">
                            <div style="font-size:0.7rem; font-weight:800; color:#334155; margin-bottom:0.35rem; text-align:center;">
                                ${d.total > 0 ? formatCLP(d.total, true) : '$0'}
                            </div>
                            <div style="width:100%; max-width:42px; height:${heightPct}%; background:${barBg}; border-radius:0.5rem 0.5rem 0 0; transition:all 0.3s; cursor:pointer;"
                                 title="${d.dayName} (${d.dateStr}): ${formatCLP(d.total)}">
                            </div>
                            <div style="font-size:0.72rem; font-weight:${isToday ? '800' : '600'}; color:${isToday ? '#4f46e5' : '#64748b'}; margin-top:0.4rem;">
                                ${d.dayName}
                            </div>
                        </div>
                    `;
    ...
    _renderPaymentMethods(salesTotal) {
        const methods = [
            ...
            { label: 'QR / Digital',  icon: '📱', amount: this.data.cashStatus.qr,            color: '#8b5cf6', bg: '#f5f3ff', border: '#c4b5fd' }
        ];
    ...
    async loadDashboardData() {
        try {
            ...
            // Productos más vendidos y Ganancia del día
            let todaySalesNet = 0;
            let todayCostNet = 0;
            const productSales = {};
            ...
            this.data.todayProfit = Math.max(0, todaySalesNet - todayCostNet);
            ...
            // Ventas por método de pago
            this.data.cashStatus = { efectivo: 0, tarjeta: 0, transferencia: 0, qr: 0 };
            todaySales.forEach(sale => {
                ...
                } else if (method.includes('qr') || method.includes('mercadopago')) {
                    this.data.cashStatus.qr += amount;
                }
            });
            ...
            for (let i = 6; i >= 0; i--) {
                ...
                const daySales = allSales
                    .filter(s => s.date && s.date.startsWith(dateStr) && s.status !== 'cancelled')
                    .reduce((sum, s) => sum + (parseFloat(s.total) || 0), 0);
                last7Days.push({ dateStr, dayName: dayName.charAt(0).toUpperCase() + dayName.slice(1), total: daySales });
            }
            this.data.last7Days = last7Days;

            // Deuda total de clientes (fiados)
            try {
                const customers = await Customer.getAll();
                this.data.totalCustomerDebt = customers.reduce((sum, c) => sum + Math.max(0, parseFloat(c.balanceCredit || c.debt || 0)), 0);
            } catch (_) {
                this.data.totalCustomerDebt = 0;
            }
