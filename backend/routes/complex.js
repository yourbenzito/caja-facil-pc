...
router.post('/api/complex/purchase', async (req, res) => {
    ...
    try {
        if (purchase.subtotal !== undefined) {
            purchase.subtotal = roundPrice(purchase.subtotal);
        }
        if (purchase.ivaAmount !== undefined) {
            purchase.ivaAmount = Math.round(purchase.ivaAmount);
        }
        if (purchase.total !== undefined) {
            purchase.total = roundPrice(purchase.total);
        }
        ...
        const pId = await withTransaction(async () => {
            ...
            for (const it of items) {
                const p = await dbGet("SELECT stock, cost, costNeto, price FROM products WHERE id = ? AND business_id = ?", [it.productId, bid]); 
                if (!p) continue;
                ...
                const currentCostNeto = (p.costNeto !== undefined && p.costNeto !== null) ? p.costNeto : ((p.cost || 0) / 1.19);
                const currentPrice = p.price || 0;
                const newPrice = it.price !== undefined ? roundPrice(parseFloat(it.price)) : currentPrice;
                ...
                await dbRun("UPDATE products SET stock = stock + ?, cost = ?, costNeto = ?, price = ?, lastSupplierId = ? WHERE id = ? AND business_id = ?", [newQty, finalAvgGross, finalAvgNeto, newPrice, purchase.supplierId, it.productId, bid]);
            ...
            if (payment.purchaseId) {
                ...
                if (purchase) {
                    ...
                    await dbRun(
                        ...
                        [newPaid, newPaid >= total - 0.01 ? 'paid' : 'pending', date, payment.purchaseId, bid]
                    );
                }
            }
