...
- **1. Dashboard (`dashboard.js`):** Resumen visual (gráficos) de ventas, productos populares y alertas de stock.
- **2. POS (Punto de Venta) (`pos.js`):** Interfaz ultra-rápida con escáner de barras, soporte para productos por kilo, ley de redondeo y ventas al contado/fiadas.
- **3. Historial de Ventas (`sales.js`):** Listado navegable de transacciones, reemisión de tickets de venta y edición/anulación.
- **4. Productos (`products.js`):** Mantenedor del catálogo con alertas de stock mínimo, fechas de caducidad y categorías.
- **5. Inventario y Kardex (`inventory.js`):** Control de movimientos de stock, ajustes manuales, mermas, consumo interno y valorización del inventario (costo vs venta).
- **6. Clientes y Cuentas Corrientes (`customers.js`):** Gestión de deudas de fiados, límites de crédito y abonos parciales/totales.
- **7. Proveedores (`suppliers.js`):** Directorio de contactos y cuentas por pagar comercial.
- **8. Compras y Recepción (`purchases.js`):** Registro de facturas con la Calculadora de Cajas/Bultos para desglose a costo unitario promedio.
- **9. Caja Registradora (`cash.js`):** Control de aperturas, cierres, ingresos/retiros manuales y arqueos sin doble conteo de deudas.
- **10. Gastos Operacionales (`expenses.js`):** Registro y clasificación de egresos del local (arriendos, servicios, sueldos) con presupuesto mensual.
- **11. Notas de Crédito y Devoluciones (`creditNotes.js`):** Registro oficial de devoluciones de productos con reingreso automático al stock y reembolso/saldo a favor.
- **12. Copiloto IA (`ai-copilot.js`):** Asistente conversacional inteligente para consultas de negocio en lenguaje natural.
- **13. Registro de Auditoría (`auditLogs.js`):** Visor de trazabilidad inmutable de acciones críticas (edición de precios, anulaciones, cierres de caja).
- **14. Reportes y Rentabilidad (`reports.js`):** Análisis de ventas y cálculo de la ganancia neta real descontando costo de ventas histórico y gastos operativos.
- **15. Configuración (`settings.js`):** Gestión de usuarios, datos del negocio para tickets, cambio de claves/PIN y backups JSON.

---

## ARQUITECTURA DEL SISTEMA

- **Tipo de Aplicación:** Desktop Application (Aplicación de Escritorio Offline-First).
- **Frontend:** HTML5, Vanilla JavaScript, CSS Puro. No utiliza frameworks reactivos (sin React/Vue/Angular). Se apoya en manipulación directa del DOM y arquitectura modular MVC/Repository.
- **Backend:** Node.js con Express.js, ejecutado como un proceso local por la aplicación.
- **Base de Datos:** SQLite 3 (base de datos relacional local en archivo `database.sqlite`) con fallback/sincronización a IndexedDB local.
...
- **APIs Externas / Servicios:** SDK de MercadoPago (`mercadopago.js`), Servicio de IA Conversacional.
...
- **`/js/`:** Lógica del Frontend (Vanilla JS).
  ...
  - `/services/`: Lógica de negocio del frontend (cálculos fiscales, flujos de caja, validaciones).
  - `/controllers/` y `/views/`: Manipulación del DOM, componentes y manejo de eventos.
...
- **`index.html`:** Única vista (Single Page Application - SPA) donde se inyectan dinámicamente los 15 módulos JS.
...
- **products:** Catálogo. Se relaciona con movimientos de stock, historial de precios y fechas de vencimiento (`expiryDate`).
- **sales:** Cabeceras de ventas con sus subtotales, totales y estatus. Contiene los detalles (items) en JSON.
- **payments:** Abonos o pagos realizados a ventas. (Relación 1 Venta -> Muchos Pagos).
- **purchases:** Facturas y registros de compras con soporte para desglose por bultos/cajas.
- **expenses:** Registro de gastos operacionales asociados a categorías y métodos de pago.
- **sale_returns / credit_notes:** Registro de devoluciones de productos con reingreso al stock.
- **cashRegisters:** Turnos de caja. Se abren y cierran, registrando el usuario y montos de inicio/fin.
- **cashMovements:** Todos los movimientos de dinero amarrados a un `cashRegisterId`.
...
- **auditLogs:** Historial de actividades críticas de los usuarios para trazabilidad e inspección de seguridad.
