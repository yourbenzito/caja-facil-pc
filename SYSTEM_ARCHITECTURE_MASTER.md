...
### Frontend (Single Page Application - 15 Vistas)
* **`@core/POS`:** Escáner de código de barras, venta por kg, Ley 20.956, carrito de compras, impresión de tickets.
* **`@domain/Inventory`:** Catalogo de productos, categorías, kardex, alertas de vencimiento y ajustes de mermas/consumo.
* **`@domain/Customers`:** Fichas de cliente, límites de crédito, fiados y registro de abonos.
* **`@domain/Suppliers`:** Directorio de proveedores e historial comercial.
* **`@domain/Purchases`:** Recepción de facturas con Calculadora de Cajas/Display para desglose a costo unitario.
* **`@domain/Expenses`:** Registro de gastos operacionales, categorías visuales y control de presupuestos.
* **`@domain/CreditNotes`:** Gestión oficial de devoluciones de clientes, restitución de saldo y reingreso a stock.
* **`@domain/Cash`:** Apertura, cierre de turnos, registro de movimientos y arqueo de caja registradora.
* **`@core/AICopilot`:** Asistente conversacional IA para consultas rápidas de negocio en tiempo real.
* **`@security/AuditLogs`:** Visor inmutable de auditoría para rastreo de operaciones críticas.
* **`@reports/Analytics`:** Paneles estadísticos, rentabilidad neta real y exportación Excel/PDF.
* **`@shared/Settings`:** Usuarios, roles, configuración de negocio y respaldos en JSON.

### Backend (Express / SQLite & Servicio REST)
* **`AuthModule`:** Autenticación JWT, hash de claves con bcrypt, recuperación de contraseña y PIN.
* **`SalesModule`:** Registro de ventas, Ley de Redondeo 20.956, desglose de métodos de pago y anulación.
* **`CreditNotesModule`:** Procesamiento de devoluciones y reintegración de stock.
* **`ExpensesModule`:** Endpoints para gestión de gastos por categoría y cálculo en flujos de caja.
* **`InventoryModule`:** Control de stock, validaciones anti-negativo y kardex de movimientos.
* **`AICopilotModule`:** Procesamiento de consultas inteligentes en lenguaje natural sobre datos de ventas/stock.
* **`AuditModule`:** Captura inalterable de bitácoras de seguridad en la tabla `auditLogs`.
* **`CashModule`:** Control de cierres de caja y conciliación de saldos sin duplicaciones.
* **`ReportsModule`:** Agregaciones en SQL para cálculo de rentabilidad neta real.
